<template>
  <div>
    <div class="flex justify-between items-center mb-8">
      <div>
        <h1 class="text-3xl font-bold text-gray-800">Daftar Promo</h1>
        <p class="text-gray-500 mt-2">Kelola brosur dan info promo terbaru.</p>
      </div>
      <NuxtLink
        to="/dashadmin/promo/create"
    ...
    <div
      ...
      <p class="text-2xl font-bold text-gray-400 mb-2">Belum ada info promo</p>
    </div>
  </div>
</template>
...
const { getPromoList, deletePromo } = usePromo();
const promoList = ref<any[]>([]);

onMounted(async () => {
  promoList.value = await getPromoList();
});
